...
function checkSectionInView() {
    ...
    if (rect.top <= windowHeight && rect.bottom >= 0) {
        ...
        window.removeEventListener('scroll', debouncedCheck); // Remove scroll listener after triggering
    }
}
...
// Create a debounced version of the checkSectionInView function
const debouncedCheck = debounce(checkSectionInView, 100);
...
const navItems = document.querySelectorAll('.left p');
    const scrollContainer = document.getElementById('scroll-container');
    const scrollSections = scrollContainer.children;

    // Function to scroll to an item and update the active state
    function scrollToItem(index) {
      const itemWidth = scrollSections[0].offsetWidth;
    ...
    // Detect scrolling and update active navigation dynamically
