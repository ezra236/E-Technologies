...
// First section: displaySection
const displaySection = document.querySelector('.display');

// Function to check if the displaySection is in view
function checkDisplaySectionInView() {
    const rect = displaySection.getBoundingClientRect();
    const windowHeight = window.innerHeight;

    if (rect.top <= windowHeight && rect.bottom >= 0) {
        displaySection.classList.add('in-view'); // Add class to trigger effect
        window.removeEventListener('scroll', debouncedDisplayCheck); // Remove scroll listener after triggering
    }
}

// Debounce function to optimize performance for displaySection
function debounceDisplay(func, delay) {
    let timeout;
...
// Create a debounced version of the checkDisplaySectionInView function
const debouncedDisplayCheck = debounceDisplay(checkDisplaySectionInView, 100);

// Listen to the scroll event for displaySection
window.addEventListener('scroll', debouncedDisplayCheck);

// Call the function initially in case displaySection is already in view
document.addEventListener('DOMContentLoaded', checkDisplaySectionInView);
...
function checkContainerInView() {
    ...
    if (rect.top <= windowHeight && rect.bottom >= 0) {
        container.classList.add('in-view'); // Add class to trigger fade-in effect
    }
}

// Debounce function to optimize performance for container
function debounceContainer(func, delay) {
    let timeout;
    return function (...args) {
        clearTimeout(timeout);
        timeout = setTimeout(() => func.apply(this, args), delay);
...
// Create a debounced version of the checkContainerInView function
const debouncedContainerCheck = debounceContainer(checkContainerInView, 100);

// Listen to the scroll event for container
window.addEventListener('scroll', debouncedContainerCheck);

// Call the function initially in case container is already in view
document.addEventListener('DOMContentLoaded', checkContainerInView);
...
// Third section: image-sectionl
const imageSectionl = document.querySelector('.image-sectionl');

// Function to check if the imageSectionl is in view
function checkImageSectionInView() {
    const rect = imageSectionl.getBoundingClientRect();
    const windowHeight = window.innerHeight;

    if (rect.top <= windowHeight && rect.bottom >= 0) {
        imageSectionl.classList.add('in-view'); // Add class to trigger fade-in effect
    }
}

// Debounce function to optimize performance for imageSectionl
function debounceImage(func, delay) {
    let timeout;
    return function (...args) {
        clearTimeout(timeout);
        timeout = setTimeout(() => func.apply(this, args), delay);
...
// Create a debounced version of the checkImageSectionInView function
const debouncedImageCheck = debounceImage(checkImageSectionInView, 100);

// Listen to the scroll event for imageSectionl
window.addEventListener('scroll', debouncedImageCheck);

// Call the function initially in case imageSectionl is already in view
document.addEventListener('DOMContentLoaded', checkImageSectionInView);
...
// Fourth section: image-sectionx
const imageSectionx = document.querySelector('.image-sectionx');

// Function to check if the imageSectionx is in view
function checkImageSectionxInView() {
    const rect = imageSectionx.getBoundingClientRect();
    const windowHeight = window.innerHeight;

    if (rect.top <= windowHeight && rect.bottom >= 0) {
        imageSectionx.classList.add('in-view'); // Add class to trigger fade-in effect
    }
}

// Debounce function to optimize performance for imageSectionx
function debounceImagex(func, delay) {
    let timeout;
    return function (...args) {
        clearTimeout(timeout);
        timeout = setTimeout(() => func.apply(this, args), delay);
...
// Create a debounced version of the checkImageSectionxInView function
const debouncedImagexCheck = debounceImagex(checkImageSectionxInView, 100);

// Listen to the scroll event for imageSectionx
window.addEventListener('scroll', debouncedImagexCheck);

// Call the function initially in case imageSectionx is already in view
document.addEventListener('DOMContentLoaded', checkImageSectionxInView);
